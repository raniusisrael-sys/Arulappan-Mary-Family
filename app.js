...
/* ✅ FIXED: Wipes internal UI explicitly so there are no ghost lines remaining */
function buildTreeChartUI() {
    const container = document.getElementById("html-tree-container");
    if (!container) return;
    
    // Wipe the container fully before trying to rebuild it
    container.innerHTML = ""; 
...
/* ✅ FIXED: Safely deletes from Google Sheets BEFORE wiping and updating the UI */
window.triggerDeleteActionLocal = function() {
    ...
    if(confirm(`Are you sure you want to completely remove family member (ID: ${activeId}) from the cloud?`)) {
        ...
        // Turn on the loading spinner so the user knows it's talking to the cloud
        document.getElementById("loading-overlay").style.display = "flex";
        document.getElementById("loading-overlay-text").innerText = "Deleting from Cloud...";

        // Send the DELETE command to your Google Apps Script
        ...
        .then(data => {
            ...
            // Remove from the local array ONLY after cloud success
            familyDataArray = familyDataArray.filter(item => item.id !== activeId);
            
            // Instantly wipe and rebuild the active cache
            familyMembersCache = {};
            familyDataArray.forEach(item => familyMembersCache[item.id] = item);
