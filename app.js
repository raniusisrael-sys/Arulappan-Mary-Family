...
// 🎯 NEW: Tracks who is in the center of the Pedigree view
let activeTreeCenterId = null; 
...
function processFamilyArrayPacket(rawData) {
    ...
    // 🎯 Set a default center if we don't have one
    if (!activeTreeCenterId && familyDataArray.length > 0) {
        // Try to find the oldest patriarch, otherwise just use the first entry
        const roots = familyDataArray.filter(m => !m.fatherId);
        activeTreeCenterId = roots.length > 0 ? roots[0].id : familyDataArray[0].id;
    }
...
/* ==========================================================================
   🌳 NEW PEDIGREE TREE LOGIC (Option 3)
   ========================================================================== */
window.recenterTree = function(newId) {
    triggerAppHapticBump();
    activeTreeCenterId = newId;
    buildTreeChartUI();
};

function getPedigreeCardHTML(m, isCenter = false) {
    if (!m) return "";
    ...
    let style = "";
    if (m.isDeceased) style += "opacity: 0.7; border-left-color: #64748b;";
    if (isCenter) style += "box-shadow: 0 0 0 3px #00a884; transform: scale(1.05); z-index: 10;";

    return `
        <div class="node-card ${isFemale ? 'female' : ''}" style="${style}" data-search-name="${m.name.toLowerCase()}">
            <img src="${avatar}" crossorigin="anonymous" alt="avatar" class="node-avatar" onclick="openProfileModal('${m.id}'); event.stopPropagation();">
            <div class="node-info" onclick="recenterTree('${m.id}')">
                <div class="node-name">${memorialBadge}${m.name}</div>
                <div class="node-title">${m.title}</div>
                ${!isCenter ? `<div class="tap-hint">👆 Tap to expand tree</div>` : `<div class="tap-hint" style="color: #64748b;">✨ Active Profile</div>`}
            </div>
        </div>
    `;
}
...
function buildTreeChartUI() {
    ...
    // Clear out any old HTML memory completely before building the new tree
    container.innerHTML = ""; 
    if (familyDataArray.length === 0 || !activeTreeCenterId) return;

    const centerNode = familyMembersCache[activeTreeCenterId];
    if (!centerNode) return;

    // Find Parents
    const father = centerNode.fatherId ? familyMembersCache[centerNode.fatherId] : null;
    let mother = null;
    if (father && father.spouseId) {
        mother = familyMembersCache[father.spouseId];
    }

    // Find Spouses
    const spouses = familyDataArray.filter(m => m.spouseId === activeTreeCenterId || (centerNode.spouseId && m.id === centerNode.spouseId));

    // Find Children (Checks if active is father OR if active is spouse of the father)
    const children = familyDataArray.filter(m => {
        const isFather = m.fatherId === activeTreeCenterId;
        const isMother = centerNode.spouseId && m.fatherId === centerNode.spouseId;
        const isWife = spouses.some(spouse => m.fatherId === spouse.id);
        return isFather || isMother || isWife;
    });

    // Build the 3-Row Layout
    let finalHTML = `<div class="pedigree-view-container">`;

    // 1. TOP ROW (Parents)
    if (father || mother) {
        finalHTML += `<div class="pedigree-row-label">⬆️ Previous Generation</div>`;
        finalHTML += `<div class="pedigree-row">`;
        if (father) finalHTML += getPedigreeCardHTML(father);
        if (mother) finalHTML += getPedigreeCardHTML(mother);
        finalHTML += `</div>`;
        finalHTML += `<div class="pedigree-connector-line"></div>`;
    }

    // 2. MIDDLE ROW (Active Node + Spouse)
    finalHTML += `<div class="pedigree-row-label">🎯 Current</div>`;
    finalHTML += `<div class="pedigree-row">`;
    finalHTML += getPedigreeCardHTML(centerNode, true); // True makes it highlighted
    spouses.forEach(s => {
        finalHTML += getPedigreeCardHTML(s);
    });
    finalHTML += `</div>`;

    // 3. BOTTOM ROW (Children)
    if (children.length > 0) {
        finalHTML += `<div class="pedigree-connector-line"></div>`;
        finalHTML += `<div class="pedigree-row-label">⬇️ Next Generation (${children.length})</div>`;
        finalHTML += `<div class="pedigree-children-row">`;
        children.forEach(child => {
            finalHTML += getPedigreeCardHTML(child);
        });
        finalHTML += `</div>`;
    } else {
        finalHTML += `<div class="pedigree-connector-line" style="background-color: transparent;"></div>`;
        finalHTML += `<div class="pedigree-row-label" style="text-transform: none; opacity: 0.6;">No children registered</div>`;
    }
...
window.triggerDeleteActionLocal = function() {
    ...
    if(confirm(`Are you sure you want to completely remove family member (ID: ${activeId}) from the cloud?`)) {
        
        document.getElementById("loading-overlay").style.display = "flex";
        document.getElementById("loading-overlay-text").innerText = "Deleting from Cloud...";
        ...
        .then(data => {
            ...
            // If we deleted the person we were looking at, reset the tree center!
            if (activeTreeCenterId === activeId) {
                activeTreeCenterId = null;
                const roots = familyDataArray.filter(m => !m.fatherId);
                if (roots.length > 0) activeTreeCenterId = roots[0].id;
                else if (familyDataArray.length > 0) activeTreeCenterId = familyDataArray[0].id;
            }
        ...
        .catch(err => {
            document.getElementById("loading-overlay").style.display = "none";
            alert("Error deleting from cloud: " + err);
        });
    }
};
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (treeSearchInput) {
        treeSearchInput.addEventListener("input", (e) => {
            ...
            // 🎯 NEW: Smart Search that auto-centers on the first match!
            const firstMatch = familyDataArray.find(m => m.name.toLowerCase().includes(query));
            if (firstMatch && activeTreeCenterId !== firstMatch.id) {
                recenterTree(firstMatch.id);
            }

            // Highlight matches
            setTimeout(() => {
                const newCards = document.querySelectorAll(".node-card");
                newCards.forEach(card => {
                    const nameAttr = card.getAttribute("data-search-name") || "";
                    if (nameAttr.includes(query)) { card.classList.remove("search-dimmed"); card.classList.add("search-match-highlight"); } 
                    else { card.classList.remove("search-match-highlight"); card.classList.add("search-dimmed"); }
                });
            }, 50);
        });
    }
